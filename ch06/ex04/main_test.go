...
func TestIntSet_Elem(t *testing.T) {
	tests := []struct {
		name       string
		fields     []int
		wantResult []uint64
	}{
		{"Elem test", []int{1, 2, 3}, []uint64{1, 2, 3}},
		{"Elem test", []int{1, 2, 3, 4, 5, 6}, []uint64{1, 2, 3, 4, 5, 6}},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			s := &IntSet{}
			s.AddAll(tt.fields...)
			if gotResult := s.Elem(); !reflect.DeepEqual(gotResult, tt.wantResult) {
				t.Errorf("IntSet.Elem() = %v, want %v", gotResult, tt.wantResult)
			}
		})
	}
}
